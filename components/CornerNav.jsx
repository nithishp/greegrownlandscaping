...
const LinksContainer = ({ setActive, isLoggedIn, handleLogout }) => {
  return (
    <motion.div className="space-y-4 p-12 pl-4 md:pl-20">
      ...
      {/* Conditionally render dashboard and logout links if the user is logged in */}
      {isLoggedIn && (
        <>
          <NavLink href="/dashboard" idx={LINKS.length} setActive={setActive}>
            Dashboard
          </NavLink>
...
const NavLink = ({ children, href, idx, setActive }) => {
    return (
      <motion.a
        ...
        exit={{ opacity: 0, y: -8 }}
        href={href}
        onClick={() => setActive(false)} // Close the menu when a link is clicked
        className="block text-5xl font-semibold text-green-400 transition-colors hover:text-violet-50 md:text-7xl"
      >
        {children}.
      </motion.a>
    );
...
const Logo = () => {
  ...
  return (
    ...
    >
      <img src="/logo-symbol.png" alt="LOGO" className="p-4" />
    </motion.a>
  );
};
  ...
  return (
    <>
      <motion.div
        ...
        className="fixed z-30 rounded-xl bg-gradient-to-br from-green-600 to-green-500 shadow-lg shadow-violet-800/20"
      />
